...
class _NegotiationState extends State<Negotiation> {
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
        ...
        onRefresh: () async {
          controller.getNegotiationCarsData(1);
          controller.getLostDeal(1);
        },
        child: Obx(() => Column(
          ...
          children: [
            ...
            Expanded(
              child: (controller.isNegotiation.value == true && (controller.carListResponse.value.data != null) && controller.carListResponse.value.data!.isNotEmpty)
                  ? GridView.builder(
                      // physics: const NeverScrollableScrollPhysics(),
                      // shrinkWrap: true,
                      controller: controller.negotiationScrollController,
                      padding: const EdgeInsets.fromLTRB(16,0,16,0),
                      itemCount: controller.searchNegotiationList.length + ((controller.searchNegotiationController.text.isNotEmpty)?0:1).toInt(),
                      ...
                      itemBuilder: (context, index) {
                        return Obx(() {
                          if(index>=controller.carListResponse.value.data!.length){
                                  if(controller.negotiationLoadingMore.value == true){
                  ...
                  : (controller.isNegotiation.value == false && controller.searchLostList.isNotEmpty)
                      ? Obx(
                          () => GridView.builder(
                            controller: controller.lostDealScrollController,
                              padding: const EdgeInsets.fromLTRB(16,0,16,0),
                              // physics: const NeverScrollableScrollPhysics(),
                              // shrinkWrap: true,
                              itemCount: controller.searchLostList.length + ((controller.searchLostController.text.isNotEmpty)?0:1).toInt(),
                              gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                                crossAxisCount: 2,
                                crossAxisSpacing: 22,
                                mainAxisSpacing: 18,
                              ...
                              itemBuilder: (context, index) {
                                 if(index>=controller.lostDealsData.value.data!.length){
                                  if(controller.lostDealLoadingMore.value == true){
                                    return Center(child: CircularProgressIndicator());
                                  } else {
                                    return const SizedBox();
                                    // return Center(child: Text(MyStrings.noMoreData, style: MyStyles.black115400,));
