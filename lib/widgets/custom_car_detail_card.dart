...
class _CustomCarDetailCardState extends State<CustomCarDetailCard> {
  ...
  void updateLikedCar(bool like) async {
    try {
      var response = await ApiManager.patch(endpoint: EndPoints.status + widget.carId, body: {"status": like == true ? "LikedCar" : "Unlike"});
                      ...
                      children: [
                        ...
                        if ((widget.bidAmount.isNotEmpty && widget.isScheduled?.value == false) || widget.bidStatus.value == CarStatus.OTB_SCHEDULED.name)
                          Obx(
                              ...
                              text: TextSpan(
                                children: <TextSpan>[
                                  TextSpan(text: '₹ ', style: MyStyles.white16700.copyWith(fontFamily: 'Rupee')),
                                  TextSpan(
                                    text: globals.documentStatus == DocumentStatus.VERIFIED.name ? widget.bidAmount.value : widget.bidAmount.value.replaceAllMapped(RegExp(r'\d'), (match) => "*").replaceAll('.', ','),
                ...
                Row(
                  children: [
                    ...
                    Padding(
                      ...
                      child: Align(alignment: Alignment.topLeft, child: Text(widget.yearOfManufacture.isNotEmpty ? widget.yearOfManufacture.substring(widget.yearOfManufacture.length - 4) : '', style: MyStyles.subTitleBlackStyle)),
                    ),
                    Padding(
                      padding: const EdgeInsets.only(left: 10),
                      child: Align(alignment: Alignment.topLeft, child: Text(widget.carModel, style: MyStyles.subTitleBlackStyle)),
                    ),
                    const SizedBox(
                      width: 2,
                    ),
                    if (widget.criticalIssue!.isNotEmpty && widget.criticalIssue!.toLowerCase().contains('good')) SvgPicture.asset(MyImages.verified)
                  ],
                ),
                Padding(
                              ...
                              children: [
                                Text(
                                  widget.isScheduled?.value == true
                                      ...
                                      : MyStrings.lastCall,
                                  style: TextStyle(
                                    color: widget.isScheduled?.value == true
                                ...
                                Row(
                                  children: [
                                    if (time.min != 0 && widget.timerController?.value != null)
                                      Icon(
                                        Icons.timer_sharp,
                                        color: widget.isScheduled!.value
                                            ? MyColors.kPrimaryColor
                                            : (time.hours != null && time.hours != 0)
                                            ? MyColors.green2
                                            : (time.min ?? 0) <= 2
                              ...
                              Text.rich(
                                TextSpan(
                                  children: [
                                    ...
                                    TextSpan(text: "₹", style: MyStyles.grey14700.copyWith(fontFamily: 'Rupee')),
