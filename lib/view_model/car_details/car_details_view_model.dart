...
class CarDetailsScreenViewModel extends GetxController {
  ...
  bool get isSliverAppBarExpanded {
    return scrollController.hasClients && scrollController.offset > ((criticalIssue.isEmpty) ? 380 : 600 - kToolbarHeight);
  }
  ...
  List<String> goodListData = ["good", "normal", "condition ok", "no blowby", "front rh power window working", "rear rh power window working", "front lh power window working", "rear lh power window working", "all windows working", "yes", "25-50%", "50-75%", "75%-100%", "working", "tilt", "telescopic", "available", "height adjustable", "functioning", "hydraulic", "electric", "remote lock", "central lock", "touch stereo", '1', '2', '2+', 'cooled glove box available'];
  ...
  List<String> yellowListData = [
    ...
    "leakage from tappet cover oil",
    "leakage from side cover oil",
    "low oil",
  ...
  void getReport() async {
    ...
    try {
      ...
      var response = await ApiManager.get(endpoint: EndPoints.evaluation + EndPoints.report + id);
      ...
      if (response.statusCode == 200) {
        ...
        if (reportResponse.value.data != null) {
          ...
          documents.value = [
            ...
            if ((reportResponse.value.data!.rcMismatch != null) && (reportResponse.value.data!.rcMismatch!.isNotEmpty)) Master(title: MyStrings.rcMismatch, value: reportResponse.value.data!.rcMismatch ?? ''),
            if ((reportResponse.value.data!.insurance != null) && (reportResponse.value.data!.insurance!.isNotEmpty)) Master(title: MyStrings.insurance, value: reportResponse.value.data!.insurance ?? ''),
            if ((reportResponse.value.data!.ncb != null) && (reportResponse.value.data!.ncb!.isNotEmpty)) Master(title: MyStrings.ncb, value: reportResponse.value.data!.ncb ?? ''),
            Master(title: MyStrings.underHypothecation, value: reportResponse.value.data!.hypothecation ?? ''),
            if ((reportResponse.value.data!.loanStatus != null) && (reportResponse.value.data!.loanStatus!.isNotEmpty)) Master(title: MyStrings.loanClosed, value: reportResponse.value.data!.loanStatus ?? ''),
            if ((reportResponse.value.data!.loanNoc != null) && (reportResponse.value.data!.loanNoc!.isNotEmpty)) Master(title: MyStrings.nocIssued, value: reportResponse.value.data!.loanNoc ?? ''),
            if ((reportResponse.value.data!.form35 != null) && (reportResponse.value.data!.form35!.isNotEmpty)) Master(title: MyStrings.form35, value: reportResponse.value.data!.form35 ?? ''),
            if ((reportResponse.value.data!.insuranceValidity != null) && (reportResponse.value.data!.insuranceValidity!.isNotEmpty)) Master(title: MyStrings.insuranceValidity, value: reportResponse.value.data!.insuranceValidity ?? ''),
          ];
          ...
          interiorAndElectrical.value = [
            ...
            Master(
              title: MyStrings.cngLpgKitImage,
              image: reportResponse.value.data!.allCarInfo!.cngKitImage?.url,
              listValue: reportResponse.value.data!.allCarInfo!.cngKitImage?.condition,
              remarks: reportResponse.value.data!.allCarInfo!.cngKitImage?.remarks,
            ),
          ...
          if (reportResponse.value.data!.allCarInfo!.startVideo != null && reportResponse.value.data!.allCarInfo!.startVideo!.url != null) {
            videoController.value = VideoPlayerController.networkUrl(Uri.parse(reportResponse.value.data!.allCarInfo!.startVideo!.url ?? ''))..initialize().then((_) {});
          }
          separateListData();
          addImageList();
          update();
  ...
  void getCarDetails() async {
    try {
      ...
      var response = await ApiManager.get(endpoint: EndPoints.carBasic + '/' + id);
      if (response.statusCode == 200) {
        ...
        timerController!.value = carDetailsResponse.value.data?[0].status == CarStatus.OTB_SCHEDULED.name
            ? CountdownTimerController(
                endTime: DateTime.now().millisecondsSinceEpoch + Duration(seconds: DateTime.parse(carDetailsResponse.value.data![0].otbStartTime ?? DateTime.now().toString()).toLocal().difference(DateTime.now()).inSeconds).inMilliseconds,
                onEnd: () {},
              )
            : carDetailsResponse.value.data?[0].status?.toLowerCase() == CarStatus.otb.name
                ? CountdownTimerController(
                    endTime: DateTime.now().millisecondsSinceEpoch + Duration(seconds: DateTime.parse(carDetailsResponse.value.data![0].otbEndTime ?? DateTime.now().toString()).toLocal().difference(DateTime.now()).inSeconds).inMilliseconds,
                    onEnd: () {},
                  )
                : carDetailsResponse.value.data?[0].status?.toLowerCase() == CarStatus.scheduled.name
                    ? CountdownTimerController(
                        endTime: DateTime.now().millisecondsSinceEpoch + Duration(seconds: DateTime.parse(carDetailsResponse.value.data![0].bidStartTime ?? DateTime.now().toString()).toLocal().difference(DateTime.now()).inSeconds).inMilliseconds,
                        onEnd: () {},
                      )
                    : CountdownTimerController(
                        endTime: DateTime.now().millisecondsSinceEpoch + Duration(seconds: DateTime.parse(carDetailsResponse.value.data![0].bidEndTime ?? DateTime.now().toString()).toLocal().difference(DateTime.now()).inSeconds).inMilliseconds,
                        onEnd: () {},
                      );
      } else {
  ...
  Future<void> updateLikedCar(bool like) async {
    try {
      ...
      var response = await ApiManager.patch(endpoint: EndPoints.status + id, body: {"status": like == true ? "LikedCar" : "Unlike"});
